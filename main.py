...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        await ctx.send("Böyle bir komut bulunamadı. Komutları görmek için `!info` yazabilirsin.")
        return
...
async def new_project(ctx):
    ...
    await ctx.send("Lütfen proje açıklamasını girin!")
...
async def update_projects(ctx):
    ...
    if projects:
        ...
        if attribute.content == 'Proje durumu':
            ...
            if update_info.content not in status_names:
